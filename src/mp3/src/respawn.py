...
y = float(sys.argv[2])
x = float(sys.argv[1])
path = (os.path.abspath(os.path.join(os.path.dirname( os.getcwd() ), '..', 'mp3')))
...
try:
    ...
    spawner("polaris", open(path + "/urdf/polaris.urdf",'r').read(), "polaris", Pose(position= Point(x,y,1),orientation=Quaternion(0,0,0,0)),"world")
except rospy.ServiceException as e:
    print("Service call failed: ",e)
